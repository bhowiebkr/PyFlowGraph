...
class NodeGraph(QGraphicsScene):
    ...
    def keyPressEvent(self, event: QKeyEvent):
        if event.key() == Qt.Key_Delete:
            for item in list(self.selectedItems()):
                if isinstance(item, (Node, RerouteNode)): self.remove_node(item)
                elif isinstance(item, Connection): self.remove_connection(item)
        else:
    ...
    def deserialize(self, data, offset=QPointF(0,0)):
        ...
        for node_data in data.get("nodes", []):
            ...
            if is_reroute:
                ...
                if 'title' in colors: node.color_title_bar = QColor(colors['title'])
                if 'body' in colors: node.color_body = QColor(colors['body'])
                node.update()
    ...
    def create_connection(self, start_pin, end_pin):
        if start_pin.can_connect_to(end_pin):
            ...
            if isinstance(end_pin.node, RerouteNode): end_pin.node.update_color()
            return conn
        return None
    def remove_connection(self, connection):
        ...
        if end_pin and isinstance(end_pin.node, RerouteNode): end_pin.node.update_color()
